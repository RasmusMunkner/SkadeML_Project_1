...
Task_Middelskade <- ReadData("claimsize_df") %>% 
...
elasticnet_tuned$train(Task_Middelskade)
...
xgb_tuned$train(Task_Middelskade)
...
ranger_tuned_skd$train(Task_Middelskade)

#Collecting results
results <- ReadData("claimsize_df") %>% 
  mutate(Pred_SkaGLMNET = elasticnet_tuned$predict(Task_Middelskade)$response,
         Pred_SkaXGB = xgb_tuned$predict(Task_Middelskade)$response,
         Pred_SkaRF = ranger_tuned_skd$predict(Task_Middelskade)$response) %>% 
  dplyr::select(ClaimAmount, Pred_SkaGLMNET, Pred_SkaXGB, Pred_SkaRF)
